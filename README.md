...
- **主界面即画布**：滚轮 / 双指缩放（以指针为锚点、按滚动量平滑缩放）、拖拽平移带惯性滑行、小地图导航，一键「看全族」「回到我」
- **经典族谱连线**：夫妻横线相连，从横线中点垂落总线，分叉到每个子女的顶边中点
- **代际横带**：祖辈 / 父辈 / 同辈 / 子辈 / 孙辈一目了然
- **亲系着色与筛选**：父系、母系、直系、后裔按色彩区分，可一键过滤；世代范围 ±1 到 ±10 代步进可调
- **五服上屏**：每张卡片带五服徽标（斩衰到缌麻描边逐级变淡，出服灰化），还可一键「只看五服内」
- **移动端防误触**：位移超过阈值判定为拖拽，抬手自动吞掉 click，平移画布不会误开人物详情；触控按钮 40px 起步

### 🔗 添加关系

- 六种细分关系：父亲 / 母亲 / **丈夫 / 妻子** / 儿子 / 女儿；男性不能添加丈夫、女性不能添加妻子（性别未知不设限）
- 选择关系即预填被添加人性别：父 / 夫 / 子 → 男，母 / 妻 / 女 → 女
- 可新建人物，也可关联已有成员（候选按关系与性别自动过滤）；自动回填缺失的另一位亲长，有歧义时逐条请你确认，**绝不凭空捏造祖先**
...
- `pnpm dev` 且本机无数据时，自动载入 `public/laotagong-*.json` 示例数据（已 gitignore，含真实亲属信息的个人导出不进版本库）；「设置」里有「重载示例数据」按钮
- 称谓词表按浙江三门县用法整理，姻亲多跳（连襟、妯娌等）由关系路径实时推导
...
pnpm test       # vitest + jsdom，167 个测试
pnpm test:watch
```
...
components/
  family-tree.tsx     画布：缩放 / 平移 / 惯性 / 小地图 / 代际横带 / 连线渲染 / 防误触
  family-app.tsx      应用外壳与各弹窗（设置含显示偏好与数据管理）
lib/
  family.ts           纯函数数据层：关系写入、关系选项规则、对称回填、关系距离、五服、生肖
  tree.ts             布局与连线几何（纯函数，逐点采样测试守卫）
  lineage.ts          亲系分类 / 筛选 / 族谱排序（血亲后代判定）
  kinship.ts          亲属称谓推导（三门县用法，含连襟、妯娌、姻亲晚辈等兜底分桶）
  view-prefs.ts       画布显示偏好（小地图、世代范围）的持久化
  dev-sample.ts       开发模式示例数据加载
...
166 个测试覆盖数据层的核心不变量：关系**双写对称**、回填只在无歧义时发生、
关系选项的可用性与默认性别、关系距离推导、称谓词表（含连襟 / 妯娌等多跳姻亲）、
序列化往返、非法输入不抛错；布局层面有「任意两卡片不得相交」
与「连线逐点采样不得穿过卡片」的几何守卫；画布指针交互（拖拽中抬手、双指缩放、
拖拽后吞掉 click 的防误触）也有回归测试。
...
- [ ] PWA 离线使用
- [ ] 导出为图片 / 可打印的族谱排版
- [ ] 撤销 / 重做（关系写入当前不可撤销，删除有确认）
- [ ] 移动端长按卡片唤出动作菜单（替代侧边动作栏）
