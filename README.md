...
- **双布局引擎，同一套几何**：「设置」里可切换自研渲染（DOM 玻璃卡片）或 AntV G6（canvas 简化卡片）——两者共用同一布局引擎，辈分行对齐与夫妻总线画法完全一致
...
components/
  family-tree.tsx     画布：缩放 / 平移 / 小地图 / 代际横带 / 连线渲染 / 防误触
  family-app.tsx      应用外壳与各弹窗
  tree-g6.tsx         G6 渲染引擎（复用自研几何，preset 定位 + 总线连线）
lib/
  ...
  layout-mode.ts      布局引擎模式（自研/G6）的持久化
...
167 个测试覆盖数据层的核心不变量：关系**双写对称**、回填只在无歧义时发生、
...
- [ ] G6 引擎的移动端捏合缩放与视口状态保持（当前重建图会丢缩放）
