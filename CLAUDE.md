...
- **弹窗里不做无谓的滚动**：字段压紧到一屏内，宁可更小。表单类弹窗
  （编辑人物 / 新建关系 / 数据管理 / 逐条指定）已全部去滚动。
  仅**无界列表**保留滚动——搜索结果、关联已有候选、全族总览：
  去掉滚动会让多出来的人彻底够不着，那是比滚动更糟的失败。
- 输入框的 `focus:ring-2` 会被 `overflow-y-auto` 容器裁掉（CSS 规范规定
  一轴为 auto 时另一轴也变 auto）。滚动容器要么加 `px-1`，要么让它不滚。
