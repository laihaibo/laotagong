...
恰好 1 位配偶 → 唯一候选，可以回填。
0 位配偶但现有子女恰好只有一位另一位亲长（共同养育）→ 同样是唯一候选，
`linkChildWithParents` 会给新子女补挂（否则共同养育家庭里新添的子女只挂单亲，
无法与兄姐同挂在父母横线下）。**≥2 位 → 是猜测，不写**，交给用户逐条指定。
...
**连线（`layout.routes`）的画法与不变量**，由 `test/routes.geometry.test.ts`
沿路径每 4px 逐点采样守卫：

标准画法（经典族谱）：双亲相邻时（已婚**或共同养育**——「添加母亲」不建
婚姻边，`buildUnits` 会把共同养育者并入同一单元实现并格，但不回写
`spouses` 数据），父母横线先相连，再从横线**中点**垂落到子女卡片
**顶边中点**；多个子女共享垂线与总线高度，长成一根主干分叉。
单亲 / 不相邻的双亲退回各自的下沿肘线，子女挂接点仍是顶边中点。

不变量：

1. 终点必须落在子女卡片顶边中点；起点要么贴在卡片边缘，
   要么（垂落画法）悬在双亲两卡之间的夫妻横线上；
2. 途中**任何一点都不得落进任何卡片内部**——卡片画在 SVG 上层，
   线被卡片盖住的那一段在视觉上就是「断线」。

踩过两次的断法：一人多配偶时 A 与 C 的婚姻线直连会横穿中间 B 的卡片
（现在从行下方 30px 绕 U 形）；亲子同排（未设「我」，或整支与「我」
不连通）时向下肘线只能从行下方爬回子女顶边、竖线整段穿过子女卡片
