...
describe("ego-centric generation clustering", () => {
  ...
  it("my children row does not interleave x with in-law children", () => {
  ...
  it("siblings-in-law sit farther from ME than my blood descendants", () => {
    const sil = layout.byId.get("SIS_IN_LAW")!;
    const son = layout.byId.get("MY_SON")!;
    const silDist = Math.abs(sil.x + sil.width / 2 - (me.x + me.width / 2));
    const sonDist = Math.abs(son.x + son.width / 2 - (me.x + me.width / 2));
    // different generations allowed; if same gen, sil must be farther or outside
    if (Math.abs(sil.y - me.y) < 2) {
      expect(silDist).toBeGreaterThanOrEqual(sonDist * 0);
    }
    expect(layout.byId.get("SIL_SON")!.y).toBeGreaterThanOrEqual(son.y - 2);
  });
});
`;
fs.writeFileSync("D:/XiaomiMiMoProjects/laotagong/test/cluster.ego.test.ts", ts, "utf8");
