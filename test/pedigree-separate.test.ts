...
describe("父母双方祖辈物理分侧", () => {
  ...
  it("通往父亲的汇合点不在母亲右侧", () => {
    const jF = layout.junctions.find((j) => j.childIds.includes("F") && !j.childIds.includes("M"));
    const jM = layout.junctions.find((j) => j.childIds.includes("M") && !j.childIds.includes("F"));
    expect(jF).toBeTruthy();
    expect(jM).toBeTruthy();
    expect(jF!.x).toBeLessThan(jM!.x);
  });
